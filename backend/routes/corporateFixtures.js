...
router.get('/my-corporate-pitches/:fixtureId', authenticateToken, authorizeRoles('bookmaker'), async (req, res) => {
    const permitNo = req.user.permitNo; // from JWT
    ...
    try {
        const [results] = await db.query(
            ...
            LEFT JOIN corporateFixturePitch cfp
                ...
                AND cfp.pitchId = p.pitchId
            WHERE u.permitNo = ?
              AND f.fixtureId = ? AND f.corporateAreaAvailable = TRUE 
            ORDER BY p.pitchNo
        `, [permitNo, fixtureId]);

        if (results.length === 0) {
            return res.status(404).json({ message: 'No corporate pitches found for this fixture' });
        }
...
router.put('/permiumFixtures/:fixtureId/:pitchId/corporate-status',authenticateToken,authorizeRoles('admin'),async (req, res) => {
        ...
        try {
            ...
            const [existing] = await db.query(
                `SELECT * FROM corporateFixturePitch WHERE fixtureId = ? AND pitchId = ?`,
                [fixtureId, pitchId]
            );
            ...
            await db.query(
                ...
                [corporateStatus, fixtureId, pitchId]
            );

            res.json({
                message: `Status updated to '${corporateStatus}' for Pitch ${pitchId} in Fixture ${fixtureId}`
            });
        } catch (err) {
            console.error(err);
            res.status(500).json({ error: err.message });
