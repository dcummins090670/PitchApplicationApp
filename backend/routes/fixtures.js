        ...
        try {
            ...
            const fixtureResult = await db.query(
                `SELECT f.fixture_date
                 ...
                 WHERE f.fixture_id = $1 AND p.pitch_id = $2 AND p.owner_permit_no = $3`,
            ...
            const existingPitch = await db.query(
                `SELECT * FROM fixture_pitch WHERE fixture_id = $1 AND pitch_id = $2`,
                [fixtureId, pitchId]
            );
            ...
            if (existing.length === 0) {
                ...
                await db.query(
                    `UPDATE fixture_pitch
                     ...
                     WHERE fixture_id = $1 AND pitch_id = $2`,
  ...
  try {
    const result = await db.query(
      `SELECT 
       ...
       WHERE f.fixture_id = $1 AND u.name !='Vacant'
       ORDER BY p.pitch_id`,
      [fixtureId]
    );
    const results = result.rows;
...
router.post("/:fixtureId/attendance-list", async (req, res) => {
        ...
        try {
            ...
            await db.query(`DELETE FROM pitch_attendance WHERE fixture_id = $1`, [fixtureId]);
  ...
  try {
    const result = await db.query(
      `SELECT 
       ...
       LEFT JOIN fixture_pitch fp
              ON fp.pitch_id = p.pitch_id AND fp.fixture_id = f.fixture_id
       WHERE f.fixture_id = $1 AND u.name !='Vacant' AND attendance = 'Attended'
       ORDER BY p.pitch_id`,
      [fixtureId]
    );
