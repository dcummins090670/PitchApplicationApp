   ...
   const { permitNo, name, phone, email, password, role } = req.body;
    ...
    try {
        // Check if permitNo already exists
         const existingNo = await db.query('SELECT * FROM users WHERE permit_no = $1', [permitNo]);
        ...
        // Insert new user
        await db.query(
            'INSERT INTO users (permit_no, name, phone, email, password_hash, role_id) VALUES ($1, $2, $3, $4, $5, $6)',
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await db.query(
            'SELECT u.permit_no, u.name, u.email, u.password_hash, r.name AS role FROM users u JOIN roles r ON u.role_id = r.id WHERE LOWER(permit_no) = LOWER($1)',
            [permitNo]
        );
