    ...
    router.get('/my-premium-pitches', authenticateToken, authorizeRoles('bookmaker'), async (req, res) => {    
        ...
        try {
            ...
            = await db.query (
                ...
                WHERE u.permit_no = $1  AND f.premium_area_available = TRUE AND f.fixture_date >= CURRENT_DATE  
  ...
  try {
    const result = await db.query(
      `SELECT 
       ...
       WHERE f.fixture_id = $1 AND u.name !='Vacant' AND pfp.premium_status = 'Applied'
       ORDER BY p.pitch_id`,
      [fixtureId]
    );
    const results = result.rows;
...
// Update pitch location for a fixture (SIS/Admin only)
    ...
    router.post("/:fixtureId/attendance-list", async (req, res) => {
        ...
        try {
            ...
            await db.query(
                `DELETE FROM premium_attendance WHERE fixture_id = $1`,
                [fixtureId]
                );
...
// Get attended pitches for a specific fixture
    router.get('/:fixtureId/awarded-pitches', async (req, res) => {
        ...
        try {
            ...
            LEFT JOIN premium_fixture_pitch pfp
                    ON pfp.pitch_id = p.pitch_id AND pfp.fixture_id = f.fixture_id
            WHERE f.fixture_id = $1 AND u.name !='Vacant' AND pfp.location ='Premium Area'
            ORDER BY p.pitch_id`,
            [fixtureId]
            );
            const results = result.rows;
