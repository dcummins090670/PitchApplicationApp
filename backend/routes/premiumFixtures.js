...
router.get('/my-premium-pitches/:fixtureId', authenticateToken, authorizeRoles('bookmaker'), async (req, res) => {
    const permitNo = req.user.permitNo; // from JWT
    ...
    try {
        const [results] = await db.query(
            ...
            LEFT JOIN premiumFixturePitch pfp
                ...
                AND pfp.pitchId = p.pitchId
            WHERE u.permitNo = ?
              AND f.fixtureId = ? AND f.premiumAreaAvailable = TRUE 
            ORDER BY p.pitchNo
        `, [permitNo, fixtureId]);

        if (results.length === 0) {
            return res.status(404).json({ message: 'No premium pitches found for this fixture' });
        }
...
// can be viewed on http://localhost:3000/my-premium-pitches
...
router.put('/permiumFixtures/:fixtureId/:pitchId/premium-status',authenticateToken,authorizeRoles('admin'),async (req, res) => {
        const { fixtureId, pitchId } = req.params;
        const { premiumStatus } = req.body;

        // Simple validation
        const validStatuses = ['Not Applying', 'Apply'];
        if (!validStatuses.includes(premiumStatus)) {
        ...
        try {
            ...
            const [existing] = await db.query(
                `SELECT * FROM PremiumFixturePitch WHERE fixtureId = ? AND pitchId = ?`,
                [fixtureId, pitchId]
            );
            ...
            await db.query(
                ...
                [premiumStatus, fixtureId, pitchId]
            );

            res.json({
                message: `Status updated to '${premiumStatus}' for Pitch ${pitchId} in Fixture ${fixtureId}`
            });
        } catch (err) {
            console.error(err);
            res.status(500).json({ error: err.message });
