...
function CorporateAttendeesPage() {
  ...
  // Fetch racecourses
    ...
    const fetchRacecourses = async () => {
        try {
            ...
            const response = await fetch("http://localhost:5000/api/corporateFixtures/racecourses", 
  ...
  const fetchAttendees = async (racecourseId) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:5000/api/corporateFixtures/${racecourseId}/attendance-list`, 
          { headers: { Authorization: `Bearer ${token}` } }
      );
