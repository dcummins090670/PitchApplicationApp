   ...
   const handlePremiumArea = async (fixtureId, newStatus, oldStatus) => {
    try {
        ...
        setFixtures((prevFixtures) =>
          prevFixtures.map((fixture) =>
            fixture.fixture_id === fixtureId
              ? { ...fixture, premium_area_available: oldStatus }
              : fixture
          )
        );
      ...
      setFixtures((prevFixtures) =>
        prevFixtures.map((fixture) =>
          fixture.fixture_id === fixtureId
            ? { ...fixture, premium_area_available: newStatus }
            : fixture
        )
      );
      ...
      console.error("Error updating Premium Area Available:", error);
      alert("Error updating Premium Area Available");
...
const handleNumberOfPitches = async (fixtureId, newValue) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/premiumFixtures/${fixtureId}/premiumArea`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          numberOfPremiumPitches: newValue,
        }),
      }
    );
    ...
    setFixtures((prev) =>
      prev.map((fx) =>
        fx.fixture_id === fixtureId
          ? { ...fx, number_of_premium_pitches: newValue }
          : fx
      )
    );

    alert("Number of premium pitches updated.");
  } catch (err) {
    console.error(err);
    alert("Error updating number of premium pitches");
  }
};
            ...
            <tbody>
              {fixtures.map((f) => (
                <tr key={f.fixture_id} className={`hover:bg-gray-100 ${
                  f.premium_area_available ? "bg-orange-200" : "bg-gray-300"
                 ...
                 <td className="border px-2 sm:px-4 py-2">{formatDate(f.fixture_date)}</td>
                  ...
                  <td className="border px-2 sm:px-4 py-2">
                   <label className="flex items-center space-x-2 cursor-pointer">
                      ...
                      />
                      <span>{f.premium_area_available ? "Yes" : " "}</span>
                    </label>
